...
exec etl_helper_station.drop_indexes('biodata');
        
prompt populating biodata station
truncate table station_swap_biodata;

insert /*+ append parallel(4) */
  into station_swap_biodata (   data_source_id, 
                                data_source, 
                                station_id, 
                                site_id, 
                                organization, 
...
select /*+ parallel(4) */ 
       ...
       nvl(station.elevation_value,
            case when biodata_site.alt_datum_cd is not null 
				then 
					case when biodata_site.altitude = '.' 
						then '0' 
						else trim(biodata_site.altitude) 
					end 
				else null 
			end
          ) elevation_value,
...
exec etl_helper_station.create_indexes('biodata');
